...
class countTable():
    def __init__(self,word=17,sparse=False,save_file=None,fromfile=False,verbose=False):
        ...
        self.max_sparse_ind = 2**64-1 #2**31#scipy.sparse decided to encoded in indeces in int32. Correct!
    ...
    def load(self):
        if self.save_file is not None:
            if self.sparse:
                self.matrix = ss.load_npz(self.save_file)#, self.matrix)
            else:
    ...
    def complete(self,verbose=False):
        ...
        if self.sparse:
            ...
            if self.max_sparse_ind<=self.max_size:
                pos_col = (pos/self.max_sparse_ind).astype(np.int64)
                pos_row = (pos-pos_col*self.max_sparse_ind).astype(np.int64)
    ...
    def consume_batch(self,batch=1000000,reset=False):
        ...
        for nm,sq in zip(self.names,self.seqs):
            for isq in range(int(len(sq)/batch)+1):
                ...
                sq_word = [sq_[i:i+word].encode('ascii') for i in range(len(sq_)-word)]
                if len(sq_word)>0:
    ...
    def consume(self,sq,verbose=False):
        ...
        word=self.word
        if len(sq)>=word:
            sq_word = [sq[i:i+word].encode('ascii') for i in range(len(sq)-word+1)]
            if verbose:
                start = time.time()
    ...
    def get(self,seq,rc=False):
        ...
        if len(seq)<word:
            return 0
        seqs = [seq[i:i+word].encode('ascii') for i in range(len(seq)-word+1)]
        if not rc:
        ...
        if self.fromfile:
            ...
            #read from RAM
            if self.sparse:
                pos_col = (ints/self.max_sparse_ind).astype(np.int64)
                pos_row = (ints-pos_col*self.max_sparse_ind).astype(np.int64)
...
class pb_reports_class:
        ...
        if files is not None:
            ...
            if self.check_extension(files,fasta_ext):
                ...
                if not force_list:
                    names =[names]
                    seqs = [seqs]
                # XXX note uses 15 here, because used for repetitive elements (rRNA and tRNA)
                OTmaps = [OTmap(seq_,word_size=15,use_kmer=use_kmer,progress_report=False,save_file=save_file, sparse=False)
                          for seq_ in seqs]
                setattr(self,map_key,OTmaps)
            elif len(files_)==1 and self.check_extension(files, ['npy']):
                # XXX Changed this to use a sparse matrix

                #print("Loading", files_[0])
                if "15w" in files_[0]:
                    OTMap_ = countTable(word=15,sparse=False,save_file=files_[0])
                else: # assume otherwise using 17mer
    ...
    def compute_pb_report_on_pbs(self, gen_seqs):
        """
        Similar to compute_pb_report, but doesn't generate the sequences -- give a list of probes and will score."
        """
        ...
        for k in range(len(gen_seqs)):
            pb_t = gen_seqs[k].replace('N','A')
            checks_ = np.zeros(len(pb_t))
    ...
    def perform_check_end(self, max_overlap=20, max_probes=None, random_select=False):
        print("Picking non-overlapping sequences.")
        start = time.time()
        self.pb_reports_keep = {}
            ...
            for key in keys_seq_sort:
                ...
                if checks_[i]==0:
                    if self.perform_check(pb_report):
                        ...
                        checks_[i:(i+self.pb_len+self.buffer_len-max_overlap)]=1
        # if only want certain number of probes, either randomly select or select by greedily maximizing distance between successive probes
        if max_probes is not None:
            temp_keep = self.pb_reports_keep.copy()
            self.pb_reports_keep = {}
            keys = list(temp_keep.keys())
            index_key = [temp_keep[k]['pb_index'] for k in keys]
            sorted_keys = np.array(keys)[np.argsort(index_key)]
            if random_select:
                keys_to_keep = sorted_keys[np.random.permutation(len(sorted_keys))[:max_probes]]
                for k in keys_to_keep:
                   self.pb_reports_keep[k] = temp_keep[k]
            else:
                # infinity distance for probe 0, then actual distances for all subsequent
                # reverse order so descending
                sorted_pb_dists_idx = np.argsort(np.concatenate([[np.inf], np.diff(index_key)]))[::-1]
                for i in sorted_pb_dists_idx[:max_probes]:
                    self.pb_reports_keep[keys[i]] = temp_keep[keys[i]]
        self.save_pbr()
        end = time.time()
        print("Time(s): "+str(end-start))
...
def partition_map(list_,map_,mapU=None,return_map=False):
    ...
    """
    list__ = np.array(list(list_))
    map__ = np.array(list(map_))
    if mapU is None:
