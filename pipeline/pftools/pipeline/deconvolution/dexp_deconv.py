...
from dexp.optics.psf import microscope_psf
from dexp.processing.utils.scatter_gather_i2i import scatter_gather_i2i
from pftools.pipeline.deconvolution.empirical_psf import nikon25x105na

import pycudadecon
import numpy 
PSF25X_405 = nikon25x105na(wvl=0.454) # Hoechst
PSF25X_560 = nikon25x105na(wvl=0.565) # Cy3
PSF25X_650 = nikon25x105na(wvl=0.665) # Cy5
PSF25X_750 = nikon25x105na(wvl=0.775) # Cy7
...
def get_pcd_deconv_fn(
    psf_kern: np.ndarray,
    num_iterations: int=5
    ):
    def decon(image):
        return pycudadecon.decon(image.to_numpy(), psf_kern, n_iters=num_iterations)
    return decon
...
def get_deconv_fn(
    ...
    def deconv(image, method='rl'):
        ...
        return lucy_richardson_deconvolution(
            ...
            psf=psf_kern.astype(np.float32),
            num_iterations=num_iterations,
            back_projection=back_proj
            #max_correction=max_correction,
            #normalise_minmax=(min_val, max_val),
            #power=power,
...
@dask.delayed
def _deconv_delayed(vol, psf, n_iter, tile_ratio):
    with CupyBackend() as bkd:
        deconv_fn = get_deconv_fn(psf, n_iter)

        margins = max(psf.shape[0], psf.shape[0])
        tiles = (vol.shape[0], int(vol.shape[1]*tile_ratio), int(vol.shape[2]*tile_ratio))
        return scatter_gather_i2i(vol, deconv_fn, margins=margins, tiles=tiles, normalise=False)

#@dask.delayed
def _deconv_pcd_delayed(vol, psf, n_iter, tile_ratio):
    deconv_fn = get_pcd_deconv_fn(psf, n_iter)
    margins = max(psf.shape[0], psf.shape[0])
    tiles = (vol.shape[0], int(vol.shape[1]*tile_ratio), int(vol.shape[2]*tile_ratio))
    return deconv_fn(vol) #scatter_gather_i2i(vol, deconv_fn, margins=margins, tiles=tiles, normalise=False)

def rl_decon_stack_pcd(vol : xr.DataArray, psf_kern: np.ndarray=None, n_iter: int=5):# -> xr.DataArray:
    """
    Takes in a volume and deconvolves using Richardson-Lucy deconv.
    Vol is xarray with labeled axis of colors.
    Will apply deconvolution to chunks. 
    """
    
    curr_t = time()
    curr_vol_arr = vol.to_numpy()
    with pycudadecon.TemporaryOTF(psf_kern) as otf:
        with pycudadecon.RLContext(vol[0].shape, otf.path) as ctx:
            for i in tqdm(range(curr_vol_arr.shape[0])):
                curr_vol_arr[i] = pycudadecon.rl_decon(curr_vol_arr[i], output_shape=curr_vol_arr[i].shape, n_iters=n_iter, background=20)
    print(f"--> Took {time() - curr_t} s ")
    return curr_vol_arr


def rl_decon_stack(vol : xr.DataArray, psf_kern: np.ndarray =None, n_iter: int=5, tile_ratio=0.25):# -> xr.DataArray:
    """
    Takes in a volume and deconvolves using Richardson-Lucy deconv.
    Vol is xarray with labeled axis of colors.
    Will apply deconvolution to chunks. 
    ...
    curr_t = time()
    lazy_computations = []
    for i, c in tqdm(enumerate(vol.color.values)):
        if psf_kern is None:
            # use theoretical
            psf = PSF_MAP[c]
        else:
            psf = psf_kern
        lazy_computations.append(_deconv_pcd_delayed(vol[i], psf, n_iter, tile_ratio))
    outputs = dask.compute(*lazy_computations) 
    #vol.values = 
    print(f"--> Took {time() - curr_t} s ")
    return da.stack([da.from_array(i.astype(np.uint16), chunks=vol[0].shape) for i in outputs])

def padded_lucy_richardson_deconvolution(
    image: xpArray,
    psf: xpArray,
    ...
    psf_nz, psf_nx, psf_ny = psf.shape
    padding_shape = ((psf_nz*2, psf_nz*2), (psf_nx*2, psf_nx*2), (psf_ny*2, psf_ny*2))
    image = np.pad(image, padding_shape, mode='constant', constant_values=0).astype(np.float32)
    ...
    initial_mask = np.pad(np.ones((nz,nx,ny)), padding_shape, mode='constant', constant_values=0).astype(np.float32)
    alpha = convolve_method(initial_mask, psf) 
    ...
    # remove padding_shape from result
    result = result[padding_shape[0][0]:-padding_shape[0][1], padding_shape[1][0]:-padding_shape[1][1], padding_shape[2][0]:-padding_shape[2][1]]
    return result