# Adapted from MERlin: 
# https://raw.githubusercontent.com/ZhuangLab/MERlin/master/merlin/data/codebook.py
...
def _parse_barcode_from_string(barcodeString: str) -> List[int]:
    """ Parse a barcode from a string.

    Args:
        barcodeString: A string of 0's and 1's representing a barcode
    Returns:
        A list of 0's and 1's representing the barcode
    """
    return [int(x) for x in barcodeString.strip()]
...
class Codebook(object):
    ...
    def __init__(self, filePath, codebookIndex: int = 0,
    ...
    def save_codebook_df(self, filePath: str):
        """ Save the codebook dataframe to a file.

        Args:
            filePath: The path to the file to save the codebook to.
        """
