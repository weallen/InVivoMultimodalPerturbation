...
def reflect(arr,sizes=[10,10,10]):
    arr_ = arr.copy()
    for axis,size in zip(list(range(len(arr.shape))),sizes):
...
from scipy.spatial import cKDTree as KDTree
def fast_fit_big_image(im,centers_zxy,radius_fit = 4,avoid_neigbors=False,recenter=False ,verbose = True,better_fit=False,troubleshoot=False):
    """
    Fast gaussian fit for big image
    Returns [Nspots x Nparams] array
    Params: hf,xc,yc,zc,bkf,w1f,w2f,w3f,t,p,eps
    """
    ps = []
...
class iter_fit_seed_points():
    def __init__(self,im,centers,radius_fit=5,min_delta_center=1.,max_delta_center=2.5,
        ...
        Returns a numpy array of size Nx(height, x, y, z, width_x, width_y,width_z,background) where N~len(centers). Bad fits are disregarded.
    ...
    def firstfit(self, verbose=True):
        """
